...
export class InventoryReviewedComponent implements OnInit, AfterViewInit, OnDestroy
{
    ...
    ngOnInit(): void
    {
        ...
        this._inventoryService.getProjects(0, 10, 'project.externalId', 'asc', '', 1, this.user.id.toString(),'R')
        .subscribe(() => {
        ...
        this.searchInputControl.valueChanges
            .pipe(
                ...
                switchMap((query) => {
                    this.closeDetails();
                    this.isLoading = true;
                    return this._inventoryService.getProjects(0, 10, 'project.externalId', 'asc', query,1, this.user.id.toString(),'R');
                }),
                map(() => {
                    this.isLoading = false;
                    this._changeDetectorRef.markForCheck();
        ...
        if ( this._sort && this._paginator )
        {
            ...
            merge(this._sort.sortChange, this._paginator.page).pipe(
                switchMap(() => {
                    this.closeDetails();
                    this.isLoading = true;
                    return this._inventoryService.getProjects(this._paginator.pageIndex, this._paginator.pageSize, this._sort.active, this._sort.direction,'',1, this.user.id.toString(),'R');
                }),
                map(() => {
                    this.isLoading = false;
                })
            ...
            if (result === 'confirmed') {
                ...
                this._inventoryService.registerEvaluation(subcriterios).subscribe(
                    () => {
                        ...
                        this._inventoryService.getProjects(this._paginator.pageIndex, this._paginator.pageSize, this._sort.active, this._sort.direction,"",1,this.user.id.toString(),'R')
                            .subscribe(() => {
                                this.isLoading = false;
                            });
                    },
